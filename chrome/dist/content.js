...
function hideContentAfterTabBar(container, tabBar) {
...
function restoreContentAfterTabBar() {
...
function buildLikesHtml(likes, profiles) {
    return likes
        .map((like) => {
...
function setupLikesInfiniteScroll(container, handle) {
    ...
    likesObserver = new IntersectionObserver((entries) => {
...
async function loadMoreLikes(handle, container) {
    if (!likesCursor)
        return;
  ...
  `;
    ...
    try {
        ...
        const response = await fetch(`https://bsky.social/xrpc/com.atproto.repo.listRecords?repo=${did}&collection=app.bsky.feed.like&limit=50&cursor=${encodeURIComponent(likesCursor)}`);
        if (!response.ok)
      ...
      <div style="text-align:center; padding:20px 20px; color:rgb(200,57,97); font-size:14px; font-family:InterVariable,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
        Error loading more likes: ${escapeHtml(error.message)}
      </div>
    `;
    }
}
...
function deselectLikesTab() {
    const likesLabel = document.querySelector('[data-testid="profilePager-Likes"]');
    if (!likesLabel)
        return;
    const likesTab = likesLabel.closest('[role="tab"]');
    if (!likesTab)
        return;
    // Reset label styling
    likesLabel.style.color = "rgb(171, 184, 201)";
    likesLabel.style.fontWeight = "600";
    // Remove blue underline
